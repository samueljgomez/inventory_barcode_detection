...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        if opt.save_crop:
                            save_one_box(xyxy, im0, file=save_dir / 'crops' / names[c] / f'{p.stem}.png', BGR=True)
            ...
            if ',' in s: print(f'{s}Done. ({t2 - t1:.3f}s)')
...
if __name__ == '__main__':
    ...
    def on_connect_local(client, userdata, flags, rc):
        if rc == 0:
            local_mqttclient.connected_flag = True
            print("Connected local OK returned code = ", rc)
            client.subscribe(LOCAL_MQTT_TOPIC)
        else:
            print("Bad connection local Returned code = ", rc)

    def on_disconnect_local(local_mqttclient, userdata, rc):
        local_mqttclient.connected_flag = False
